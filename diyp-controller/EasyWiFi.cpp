...
void EasyWiFi::start()
{
  ...
  { // check if connected
    ...
    {                  // read credentials, if not possible, re-use the old-already loaded credentials
...
void EasyWiFi::listNetworks()
{
  ...
  else
  {
    ...
    for (int thisNet = 0; thisNet < numSsid; thisNet++)
    {
      ...
      if (G_ssidCounter < MAXSSID)
      { // store only maximum of <SSIDMAX> SSDI's with high dB > -80        && WiFi.RSSI(thisNet)>-81
...
void EasyWiFi::APDNSScan()
{
  ...
  {                                                  // We've received a packet, read the data from it
...

    {
      ...
      Serial.print(") from ");
      Serial.print(G_APDNSclientip);
      Serial.print(" port ");
      Serial.println(G_DNSClientport);
      for (t = 0; t < packetSize; ++t)
      {
        Serial.print(G_UDPPacketbuffer[t], HEX);
        Serial.print(":");
      }
      Serial.println(" ");
      for (t = 0; t < packetSize; ++t)
      ...
      {
        Serial.print(G_DNSReplybuffer[t], HEX);
        Serial.print(":");
      }
      Serial.println(" ");
      for (t = 0; t < replySize; ++t)
      ...
      {                    // if there's bytes to read from the client,
        ...
        { // if the byte is a newline character
          ...
          if (currentLine.length() == 0)
          {
            ...
            client.print("<font size=5>Arduino</font>  <br><font size=5>");
        ...
        if (currentLine.endsWith("POST /checkpass.php"))
        {
          ...
          { // loop while the client's connected
            ...
            {                    // if there's bytes to read from the client,
              ...
              { // Check read line on "data=" start that ends with "&action"
                ...
                if (t < 78)
                {
                  if (pos2 - pos1 == 7)
                  {
                    ...
                    for (v = 0; G_SSIDList[u][v] != 0; v++)
                      G_ssid[v] = G_SSIDList[u][v];
                    G_ssid[v] = 0; // copy list name to ssid
                  }
                  else
                  {
                    u = 0;
                ...
                {
#ifdef DBGON
                  Serial.print("* Invalid input from AP Client");
        ...
        if (currentLine.endsWith("GET /generate_204"))
        { // HTTP generate_204 HTTP/1.1
          client.println("HTTP/1.1 200 OK");
          client.println("Content-type:text/html");
          client.println();
...
byte EasyWiFi::Read_Credentials(char *ssid, char *password)
{
  WifiCredentials cred;
  WiFiStorageFile file = WiFiStorage.open(CREDENTIALFILE);

#ifdef DBGON
  Serial.print("* Read Credentials open result: ");
  Serial.println(file);
#endif
  if ( !file )
    return  0;
  ...
  file.seek(0);
  if (file.read(&cred, sizeof(cred)) != sizeof(cred) )
  {
    file.close();
    return 0;
  }

  strncpy(ssid, cred.ssid, sizeof(cred.ssid));
  strncpy(password, cred.password, sizeof(cred.password));
...
byte EasyWiFi::Write_Credentials(char *ssid, int size1, char *password, int size2)
{

#ifdef DBGON
    Serial.print("* Write Credentials");
#endif

  WifiCredentials cred;
  WiFiStorageFile file = WiFiStorage.open(CREDENTIALFILE);
  strncpy(cred.ssid, ssid, sizeof(cred.ssid));
  strncpy(cred.password, password, sizeof(cred.password));

  if ( file.write(&cred, sizeof(cred)) != sizeof(cred) )
    return 0;
...
byte EasyWiFi::Erase_Credentials()
{
  ...
  file.erase();
#ifdef DBGON
    Serial.println("* Erased Credentialsfile : " CREDENTIALFILE);
#endif
  return 1;
}
...
byte EasyWiFi::Check_Credentials()
{
  ...
  {
#ifdef DBGON
    Serial.println("* Found Credentials file : ");
#endif
    return (1);
  }
  else
  {
#ifdef DBGON
    Serial.println("* Could not find Credentials file : ");
#endif
    return (0);
  }
}
